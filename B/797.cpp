...
int n;
vi v;
vvi cache(MAX, vi(2, -1));

int calculate(int idx, bool odd){
    if (idx == n && odd) return 0;
    if (idx == n && !odd) return -MOD;
    if (cache[idx][odd] != -1) return cache[idx][odd];
    int s1 = calculate(idx+1, odd);
    int s2 = v[idx] + calculate(idx+1, odd^(v[idx]&1));
    // trace3(idx, s1, s2);
    return cache[idx][odd] = max(s1, s2);
}
